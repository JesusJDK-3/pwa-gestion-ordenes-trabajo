...
CREATE FUNCTION public.fn_sync_geom_punto() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
ALTER FUNCTION public.fn_sync_geom_punto() OWNER TO postgres;
...
CREATE INDEX idx_gis_geom ON public.gis_punto_operativo USING gist (geom);
...
CREATE INDEX idx_ot_geom ON public.op_orden_trabajo USING gist (geom);
...
CREATE TRIGGER trg_sync_geom_punto BEFORE INSERT OR UPDATE OF latitud, longitud ON public.gis_punto_operativo FOR EACH ROW EXECUTE FUNCTION public.fn_sync_geom_punto();
